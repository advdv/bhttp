...
func BenchmarkHandler(b *testing.B) {
	...
	for _, tblc := range []struct {
		...
		b.Run("buffered-"+tblc.name, func(b *testing.B) {
			hdlr := bhttp.ServeFunc(func(ctx context.Context, w bhttp.ResponseWriter, r *http.Request) error {
				tblc.hf(w, r)

				return nil
			}, bhttp.BasicContextFromRequest())
...
var _ = Describe("handle implementations", func() {
	It("should return usual response on success", func(ctx context.Context) {
		...
		bhttp.ServeFunc(func(ctx context.Context, w bhttp.ResponseWriter, r *http.Request) error {
			w.Header().Set("X-Foo", "bar")
			w.WriteHeader(http.StatusCreated)
			fmt.Fprintf(w, "show this")

			return nil
		}, bhttp.BasicContextFromRequest()).ServeHTTP(resp, req)
	...
	It("error on writing to limit buffer", func(ctx context.Context) {
		...
		bhttp.ServeFunc(func(ctx context.Context, w bhttp.ResponseWriter, r *http.Request) error {
			...
			return nil
		}, bhttp.BasicContextFromRequest(), bhttp.WithBufferLimit(1)).ServeHTTP(resp, req)
	...
	It("should log unhandled errors by default", func(ctx context.Context) {
		...
		bhttp.ServeFunc(func(ctx context.Context, w bhttp.ResponseWriter, r *http.Request) error {
			return errSome
		}, bhttp.BasicContextFromRequest(), bhttp.WithErrorLog(logs)).ServeHTTP(resp, req)
	...
	It("should handle flushing error", func(ctx context.Context) {
		...
		bhttp.ServeFunc(func(ctx context.Context, w bhttp.ResponseWriter, r *http.Request) error {
			...
			return nil
		}, bhttp.BasicContextFromRequest(), bhttp.WithErrorLog(logs)).ServeHTTP(fresp, req)
